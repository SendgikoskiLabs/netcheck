...
Run it with no arguments to launch the dark-themed GUI. Pass a subcommand to use it from the CLI. Pipe `--json` output into your own tooling. Run `--monitor` to watch multiple hosts continuously with intelligent alerting. Run `--influx` to send results to an InfluxDB dashboard.
...
python netcheck.py ping google.com -c 10

# Full diagnostic (DNS + TCP + TLS + HTTP + ASN)
python netcheck.py check google.com

# Traceroute
python netcheck.py traceroute google.com
python netcheck.py traceroute google.com -m 20

# Run full check on all default hosts
python netcheck.py all

# Continuous monitor (default hosts, 10s interval)
python netcheck.py monitor

# Monitor specific hosts at custom interval
python netcheck.py monitor --hosts github.com cloudflare.com -i 30

# JSON output (any command)
python netcheck.py check google.com --json
...
```bash
python netcheck.py check github.com --json
```
...
3. Start `python netcheck.py monitor --influx` to stream data into InfluxDB
...
| Version | Changes |
|---|---|
| **4.0** | v4.0: Renamed tool from sendgikoski_netcheck.py to netcheck.py; Migrated to new SendgikoskiLabs/netcheck repository; Integrated sendgikoskilabs_core shared library as foundation; Archived SendgikoskiLabs/netcheck_v3 (preserved for reference) |
